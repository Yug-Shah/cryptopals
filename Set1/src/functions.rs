...
pub fn get_raw_data(hex: &str) -> String{
    std::str::from_utf8(&(hex::decode(hex).unwrap())).expect("Error in input").to_string()
}
...
pub fn fixed_xor(hex1: &str, hex2: &str) -> String {
    ...
    hex::encode(byte1.iter().zip(byte2.iter()).map(|(&b1, &b2)| b1 ^ b2).collect::<Vec<u8>>())
}

pub fn crack_xor(hex1: &str) -> (String, u8) {
...
pub fn detect_xor(path: &str) -> (String, String, u8) {
    ...
    for line in buffered.flatten(){
        let (temp, k) = crack_xor(line.as_str());
...
mod tests {
    ...
    fn test_challenge_2(){
        ...
        assert_eq!(raw_data_1, "\u{1c}\u{1}\u{11}\0\u{1f}\u{1}\u{1}\0\u{6}\u{1a}\u{2}KSSP\t\u{18}\u{1c}");
    ...
    fn test_challenge_4() {
        ...
        let (hex, result, key) = detect_xor(file_path);
        assert_eq!(hex, "7b5a4215415d544115415d5015455447414c155c46155f4058455c5b523f");
        assert_eq!(key, b'5');
